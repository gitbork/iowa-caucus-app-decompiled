...
public class FaceDetectorAsyncTask extends AsyncTask<Void, Void, SparseArray<Face>> {
    ...
    public SparseArray<Face> doInBackground(Void... voidArr) {
        if (!isCancelled() && this.mDelegate != null) {
            ...
            if (rNFaceDetector != null && rNFaceDetector.isOperational()) {
    ...
    private WritableArray serializeEventData(SparseArray<Face> sparseArray) {
        ...
        for (int i = 0; i < sparseArray.size(); i++) {
            WritableMap serializeFace = FaceDetectorUtils.serializeFace((Face) sparseArray.valueAt(i), this.mScaleX, this.mScaleY, this.mWidth, this.mHeight, this.mPaddingLeft, this.mPaddingTop);
