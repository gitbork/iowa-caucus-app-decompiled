...
public class TextRecognizerAsyncTask extends AsyncTask<Void, Void, SparseArray<TextBlock>> {
    ...
    public SparseArray<TextBlock> doInBackground(Void... voidArr) {
        ...
        this.mTextRecognizer = new Builder(this.mThemedReactContext).build();
    ...
    public void onPostExecute(SparseArray<TextBlock> sparseArray) {
        ...
        if (sparseArray != null) {
            ...
            for (int i = 0; i < sparseArray.size(); i++) {
                WritableMap serializeText = serializeText((TextBlock) sparseArray.valueAt(i));
    ...
    private WritableMap serializeText(Text text) {
        ...
        String str = text instanceof TextBlock ? "block" : text instanceof Line ? "line" : "element";
    ...
    private WritableMap rotateTextX(WritableMap writableMap) {
        ...
        String str2 = "origin";
        WritableMap positionTranslatedHorizontally = FaceDetectorUtils.positionTranslatedHorizontally(FaceDetectorUtils.positionMirroredHorizontally(map.getMap(str2), this.mImageDimensions.getWidth(), this.mScaleX), -map.getMap("size").getDouble("width"));
        WritableMap createMap = Arguments.createMap();
        createMap.merge(map);
        createMap.putMap(str2, positionTranslatedHorizontally);
        writableMap.putMap(str, createMap);
        String str3 = "components";
        ReadableArray array = writableMap.getArray(str3);
        WritableArray createArray = Arguments.createArray();
        ...
        writableMap.putArray(str3, createArray);
        return writableMap;
    }
}
