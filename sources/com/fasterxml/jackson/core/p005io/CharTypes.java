...
public final class CharTypes {
    ...
    /* renamed from: HB */
    private static final byte[] f65HB;

    /* renamed from: HC */
    private static final char[] f66HC = "0123456789ABCDEF".toCharArray();
    ...
    static {
        int length = f66HC.length;
        f65HB = new byte[length];
        for (int i = 0; i < length; i++) {
            f65HB[i] = (byte) f66HC[i];
        }
        ...
        for (int i3 = 128; i3 < 256; i3++) {
            int i4 = (i3 & CanonMakernoteDirectory.TAG_SENSOR_INFO_ARRAY) == 192 ? 2 : (i3 & 240) == 224 ? 3 : (i3 & 248) == 240 ? 4 : -1;
            iArr3[i3] = i4;
        }
        ...
        for (int i = 0; i < length2; i++) {
            ...
            if (charAt >= length || iArr[charAt] == 0) {
                ...
                if (i2 < 0) {
                    ...
                    sb.append('0');
                    sb.append('0');
                    sb.append(f66HC[charAt >> 4]);
                    sb.append(f66HC[charAt & 15]);
                } else {
                    sb.append((char) i2);
                }
            }
    ...
    public static char[] copyHexChars() {
        return (char[]) f66HC.clone();
    }

    public static byte[] copyHexBytes() {
        return (byte[]) f65HB.clone();
    }
}
