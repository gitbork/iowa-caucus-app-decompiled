...
public class ImageMetadataReader {
    ...
    public static void main(@NotNull String[] strArr) throws MetadataException, IOException {
        ...
        if (arrayList.size() < 1) {
            ...
            sb.append("metadata-extractor version ");
            ...
            printStream2.println(String.format("Usage: java -jar metadata-extractor-%s.jar <filename> [<filename>] [-thumb] [-markdown] [-hex]", objArr));
            System.exit(1);
        }
        for (String str2 : arrayList) {
            ...
            if (!remove) {
                ...
                Double.isNaN(d);
                printStream3.printf("Processed %.3f MB file in %.2f ms%n%n", new Object[]{Double.valueOf(length / 1048576.0d), Double.valueOf(d / 1000000.0d)});
            }
            if (remove) {
            ...
            for (Directory directory : metadata.getDirectories()) {
                ...
                for (Tag tag : directory.getTags()) {
                    ...
                    if (description != null && description.length() > 1024) {
                        ...
                        sb2.append(description.substring(0, 1024));
                        sb2.append("...");
                        description = sb2.toString();
                    }
                    if (remove) {
                        System.out.printf("%s|0x%s|%s|%s%n", new Object[]{name2, Integer.toHexString(tag.getTagType()), tagName, description});
                    } else if (remove2) {
                        System.out.printf("[%s - %s] %s = %s%n", new Object[]{name2, tag.getTagTypeHex(), tagName, description});
                    } else {
                        System.out.printf("[%s] %s = %s%n", new Object[]{name2, tagName, description});
                    }
                }
                for (String str4 : directory.getErrors()) {
