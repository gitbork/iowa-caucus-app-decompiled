...
public class StorageReference implements Comparable<StorageReference> {
    ...
    public Task<ListResult> listAll() {
        ...
        Task listHelper = listHelper(null, null);
        final Executor executor = commandPoolExecutor;
        final TaskCompletionSource taskCompletionSource2 = taskCompletionSource;
        C15664 r0 = new Continuation<ListResult, Task<Void>>() {
            public Task<Void> then(@NonNull Task<ListResult> task) {
                ListResult listResult = (ListResult) task.getResult();
                arrayList.addAll(listResult.getPrefixes());
                arrayList2.addAll(listResult.getItems());
                if (listResult.getPageToken() != null) {
                    StorageReference.this.listHelper(null, listResult.getPageToken()).continueWithTask(executor, this);
                } else {
                    taskCompletionSource2.setResult(new ListResult(arrayList, arrayList2, null));
                }
                return Tasks.forResult(null);
            }
        };
        listHelper.continueWithTask(commandPoolExecutor, r0);
        return taskCompletionSource.getTask();
    }
