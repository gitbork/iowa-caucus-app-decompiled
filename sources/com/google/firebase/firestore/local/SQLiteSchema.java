...
class SQLiteSchema {
    ...
    /* renamed from: db */
    private final SQLiteDatabase f252db;

    SQLiteSchema(SQLiteDatabase sQLiteDatabase) {
        this.f252db = sQLiteDatabase;
    }
    ...
    static /* synthetic */ void lambda$createV1MutationQueue$0(SQLiteSchema sQLiteSchema) {
        sQLiteSchema.f252db.execSQL("CREATE TABLE mutation_queues (uid TEXT PRIMARY KEY, last_acknowledged_batch_id INTEGER, last_stream_token BLOB)");
        sQLiteSchema.f252db.execSQL("CREATE TABLE mutations (uid TEXT, batch_id INTEGER, mutations BLOB, PRIMARY KEY (uid, batch_id))");
        sQLiteSchema.f252db.execSQL("CREATE TABLE document_mutations (uid TEXT, path TEXT, batch_id INTEGER, PRIMARY KEY (uid, path, batch_id))");
    }

    private void removeAcknowledgedMutations() {
        new Query(this.f252db, "SELECT uid, last_acknowledged_batch_id FROM mutation_queues").forEach(SQLiteSchema$$Lambda$2.lambdaFactory$(this));
    }

    static /* synthetic */ void lambda$removeAcknowledgedMutations$2(SQLiteSchema sQLiteSchema, Cursor cursor) {
        String string = cursor.getString(0);
        long j = cursor.getLong(1);
        new Query(sQLiteSchema.f252db, "SELECT batch_id FROM mutations WHERE uid = ? AND batch_id <= ?").binding(string, Long.valueOf(j)).forEach(SQLiteSchema$$Lambda$12.lambdaFactory$(sQLiteSchema, string));
    }

    /* access modifiers changed from: private */
    public void removeMutationBatch(String str, int i) {
        SQLiteStatement compileStatement = this.f252db.compileStatement("DELETE FROM mutations WHERE uid = ? AND batch_id = ?");
        ...
        this.f252db.execSQL("DELETE FROM document_mutations WHERE uid = ? AND batch_id = ?", new Object[]{str, Integer.valueOf(i)});
    }
    ...
    static /* synthetic */ void lambda$createV1QueryCache$3(SQLiteSchema sQLiteSchema) {
        sQLiteSchema.f252db.execSQL("CREATE TABLE targets (target_id INTEGER PRIMARY KEY, canonical_id TEXT, snapshot_version_seconds INTEGER, snapshot_version_nanos INTEGER, resume_token BLOB, last_listen_sequence_number INTEGER,target_proto BLOB)");
        sQLiteSchema.f252db.execSQL("CREATE INDEX query_targets ON targets (canonical_id, target_id)");
        sQLiteSchema.f252db.execSQL("CREATE TABLE target_globals (highest_target_id INTEGER, highest_listen_sequence_number INTEGER, last_remote_snapshot_version_seconds INTEGER, last_remote_snapshot_version_nanos INTEGER)");
        sQLiteSchema.f252db.execSQL("CREATE TABLE target_documents (target_id INTEGER, path TEXT, PRIMARY KEY (target_id, path))");
        sQLiteSchema.f252db.execSQL("CREATE INDEX document_targets ON target_documents (path, target_id)");
    }

    private void dropV1QueryCache() {
        if (tableExists("targets")) {
            this.f252db.execSQL("DROP TABLE targets");
        }
        if (tableExists("target_globals")) {
            this.f252db.execSQL("DROP TABLE target_globals");
        }
        if (tableExists("target_documents")) {
            this.f252db.execSQL("DROP TABLE target_documents");
        }
    }
    ...
    private void ensureTargetGlobal() {
        if (!(DatabaseUtils.queryNumEntries(this.f252db, "target_globals") == 1)) {
            String str = "0";
            this.f252db.execSQL("INSERT INTO target_globals (highest_target_id, highest_listen_sequence_number, last_remote_snapshot_version_seconds, last_remote_snapshot_version_nanos) VALUES (?, ?, ?, ?)", new String[]{str, str, str, str});
        }
    }
    ...
    private void addTargetCount() {
        ...
        if (!tableContainsColumn(str2, str)) {
            this.f252db.execSQL("ALTER TABLE target_globals ADD COLUMN target_count INTEGER");
        }
        long queryNumEntries = DatabaseUtils.queryNumEntries(this.f252db, "targets");
        ContentValues contentValues = new ContentValues();
        contentValues.put(str, Long.valueOf(queryNumEntries));
        this.f252db.update(str2, contentValues, null, null);
    }

    private void addSequenceNumber() {
        if (!tableContainsColumn("target_documents", "sequence_number")) {
            this.f252db.execSQL("ALTER TABLE target_documents ADD COLUMN sequence_number INTEGER");
        }
    }

    private void ensureSequenceNumbers() {
        Long l = (Long) new Query(this.f252db, "SELECT highest_listen_sequence_number FROM target_globals LIMIT 1").firstValue(SQLiteSchema$$Lambda$6.lambdaFactory$());
        Assert.hardAssert(l != null, "Missing highest sequence number", new Object[0]);
        long longValue = l.longValue();
        SQLiteStatement compileStatement = this.f252db.compileStatement("INSERT INTO target_documents (target_id, path, sequence_number) VALUES (0, ?, ?)");
        Query binding = new Query(this.f252db, "SELECT RD.path FROM remote_documents AS RD WHERE NOT EXISTS (SELECT TD.path FROM target_documents AS TD WHERE RD.path = TD.path AND TD.target_id = 0) LIMIT ?").binding(Integer.valueOf(100));
    ...
    private void createV8CollectionParentsIndex() {
        ...
        Consumer lambdaFactory$ = SQLiteSchema$$Lambda$9.lambdaFactory$(new MemoryCollectionParentIndex(), this.f252db.compileStatement("INSERT OR REPLACE INTO collection_parents (collection_id, parent) VALUES (?, ?)"));
        new Query(this.f252db, "SELECT path FROM remote_documents").forEach(SQLiteSchema$$Lambda$10.lambdaFactory$(lambdaFactory$));
        new Query(this.f252db, "SELECT path FROM document_mutations").forEach(SQLiteSchema$$Lambda$11.lambdaFactory$(lambdaFactory$));
    }
    ...
    public java.util.List<java.lang.String> getTableColumns(java.lang.String r6) {
        /*
            ...
            r0.<init>()
            r1 = 0
            android.database.sqlite.SQLiteDatabase r2 = r5.f252db     // Catch:{ all -> 0x003c }
    ...
    private boolean tableExists(String str) {
        return !new Query(this.f252db, "SELECT 1=1 FROM sqlite_master WHERE tbl_name = ?").binding(str).isEmpty();
    }
}
