...
public final class FirestoreClient implements RemoteStoreCallback {
    ...
    private void initialize(Context context, User user, boolean z, long j) {
        ...
        if (z) {
            ...
            SQLitePersistence sQLitePersistence = new SQLitePersistence(context2, this.databaseInfo.getPersistenceKey(), this.databaseInfo.getDatabaseId(), new LocalSerializer(new RemoteSerializer(this.databaseInfo.getDatabaseId())), Params.WithCacheSizeBytes(j));
        ...
        RemoteStore remoteStore2 = new RemoteStore(this, this.localStore, new Datastore(this.databaseInfo, this.asyncQueue, this.credentialsProvider, context), this.asyncQueue, new AndroidConnectivityMonitor(context));
