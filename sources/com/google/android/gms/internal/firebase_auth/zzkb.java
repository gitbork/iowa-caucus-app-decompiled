...
class zzkb<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public V get(Object obj) {
        ...
        if (zza2 >= 0) {
            return ((zzkg) this.zzb.get(zza2)).getValue();
        }
    ...
    public final V put(K k, V v) {
        ...
        int zza2 = zza(k);
        if (zza2 >= 0) {
            return ((zzkg) this.zzb.get(zza2)).setValue(v);
        }
        zzf();
        ...
        int i2 = this.zza;
        if (size == i2) {
            zzkg zzkg = (zzkg) this.zzb.remove(i2 - 1);
            zzg().put((Comparable) zzkg.getKey(), zzkg.getValue());
        }
    ...
    public final V zzc(int i) {
        zzf();
        V value = ((zzkg) this.zzb.remove(i)).getValue();
        if (!this.zzc.isEmpty()) {
            Iterator it = zzg().entrySet().iterator();
            this.zzb.add(new zzkg(this, (Entry) it.next()));
            it.remove();
        }
        return value;
    }
    ...
    private final int zza(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo((Comparable) ((zzkg) this.zzb.get(size)).getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo((Comparable) ((zzkg) this.zzb.get(i2)).getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;
    ...
    public int hashCode() {
        ...
        for (int i2 = 0; i2 < zzc(); i2++) {
            i += ((zzkg) this.zzb.get(i2)).hashCode();
        }
