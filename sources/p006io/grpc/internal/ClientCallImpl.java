...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private class ClientStreamListenerImpl implements ClientStreamListener {
            ...
            ClientCallImpl.this.callExecutor.execute(new ContextRunnable() {
                public final void runInContext() {
                    if (!ClientStreamListenerImpl.this.closed) {
                        String str = "ClientCall.headersRead";
                        PerfMark.taskStart(ClientCallImpl.this.tag, str);
                        try {
                            ClientStreamListenerImpl.this.observer.onHeaders(metadata);
                        } catch (Throwable th) {
                            PerfMark.taskEnd(ClientCallImpl.this.tag, str);
                            throw th;
                        }
                        PerfMark.taskEnd(ClientCallImpl.this.tag, str);
                    }
        ...
        public void messagesAvailable(final MessageProducer messageProducer) {
            ClientCallImpl.this.callExecutor.execute(new ContextRunnable() {
                public final void runInContext() {
                    ...
                    String str = "ClientCall.messagesAvailable";
                    PerfMark.taskStart(ClientCallImpl.this.tag, str);
                    while (true) {
                        try {
        ...
        public void closed(final Status status, RpcProgress rpcProgress, final Metadata metadata) {
            ...
            if (status.getCode() == Code.CANCELLED && access$1300 != null && access$1300.isExpired()) {
    ...
    private void startInternal(final Listener<RespT> listener, Metadata metadata) {
    ...
    public void cancel(@Nullable String str, @Nullable Throwable th) {
        String str2 = "ClientCall.cancel";
        PerfMark.taskStart(this.tag, str2);
        try {
            cancelInternal(str, th);
        } finally {
            PerfMark.taskEnd(this.tag, str2);
        }
    }
    ...
    public void halfClose() {
        String str = "ClientCall.halfClose";
        PerfMark.taskStart(this.tag, str);
        try {
            halfCloseInternal();
        } finally {
    ...
    public void sendMessage(ReqT reqt) {
        String str = "ClientCall.sendMessage";
        PerfMark.taskStart(this.tag, str);
        try {
